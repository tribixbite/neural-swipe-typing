...
### Alpha Model Test Script
- **File**: `test_alpha_model.py`
- **Purpose**: To validate the accuracy of the pre-existing ONNX models in the `/alpha` directory, which were generated by `export_and_quantize_standalone.py`.
- **Methodology**:
    - The script loads the `swipe_encoder_android.onnx` and `swipe_decoder_android.onnx` models.
    - It processes a sample of 100 words from the `data/val_hwsfuto.jsonl` validation set.
    - It uses a beam search decoder to generate predictions and compares them against the ground truth.
- **Status**:
    - **Completed**.
    - The script successfully runs and reports a word accuracy of **75.00%**.
    - This serves as a reliable validation test for the specific ONNX model version in the `alpha` directory.