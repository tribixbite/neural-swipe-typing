...
class MobileSwipeDecoderExecutorch(nn.Module):
    """Mobile-optimized decoder for character sequence generation - ExecuTorch Compatible"""
    def __init__(self, d_model: int, vocab_size: int = 30, max_len: int = 20, 
    ...
    def forward(self, encoder_output: torch.Tensor, target_chars: Optional[torch.Tensor] = None) -> torch.Tensor:
        if target_chars is not None:
            ...
            # Inference mode - Fixed length decoding (no dynamic EOS check)
            ...
            # Fixed-length generation for ExecuTorch compatibility
            for step in range(self.max_len):
                ...
                # Note: Removed dynamic EOS check for ExecuTorch compatibility
                # Model will generate exactly max_len characters
            
            return torch.cat(outputs, dim=1)

class MobileSwipeTypingModelExecutorch(nn.Module):
    """Complete mobile-optimized swipe typing model - ExecuTorch Compatible"""
    def __init__(self, d_model: int = 64, vocab_size: int = 30, max_seq_len: int = 150,
        ...
        # Decoder
        self.decoder = MobileSwipeDecoderExecutorch(d_model, vocab_size, max_word_len, 
...
def create_mobile_model_executorch(vocab_size: int = 30, d_model: int = 64) -> MobileSwipeTypingModelExecutorch:
    """Factory function for ExecuTorch-compatible mobile-optimized model"""
    return MobileSwipeTypingModelExecutorch(
        d_model=d_model,
...
if __name__ == "__main__":
    mobile_model = create_mobile_model_executorch()
    print(f"ExecuTorch-compatible mobile model parameters: {count_parameters(mobile_model):,}")
    
    # Test forward pass
    batch_size, seq_len, feature_dim = 4, 100, 6
    dummy_input = torch.randn(batch_size, seq_len, feature_dim)
    dummy_target = torch.randint(0, 30, (batch_size, 15))  # Max 15 chars
    ...
    with torch.no_grad():
        ...
        print("✅ ExecuTorch-compatible model working correctly")