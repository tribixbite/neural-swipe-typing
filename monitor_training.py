...
def get_latest_checkpoint():
    ...
    checkpoints = glob.glob("checkpoints/enhanced_word/*.ckpt")
    if checkpoints:
        # Extract accuracy from filename
        best_acc = 0
        best_ckpt = None
        for ckpt in checkpoints:
            # Parse accuracy from filename like: enhanced-word-05-0.456.ckpt
            try:
...
def monitor():
    ...
    print("="*60)
    print("Enhanced Word Model Training Monitor")
    print("="*60)
    print("Target: 70% accuracy (matching original model)")
    print("Previous mobile model: 26% accuracy")
    print("-"*60)
    ...
    while True:
        ...
        if acc > last_acc:
            print(f"\n✓ New best accuracy: {acc:.1%}")
            if ckpt:
                print(f"  Checkpoint: {Path(ckpt).name}")
            
            if acc >= 0.70:
                print("\n" + "="*60)
                print(f"🎉 TARGET ACHIEVED! {acc:.1%} accuracy")
                ...
                print(f"  Progress: Significant improvement over mobile model")
